...
export async function generateImage(
  ...
  try {
    ...
    return {
      imageUrl: "",
      model,
      error: `Image generation with ${model.name} is unavailable from Hugging Face right now (${errorMessage}). Retry when the provider finishes loading or configure Hugging Face access.`,
    };
  }
}
